...
def sentence1() -> Expr:
    ...
    not_a = Expr('~',a)
    not_b = Expr('~',b)
    not_b_or_c = Expr('|',not_b,c) 
    iff = Expr('<=>',not_a,not_b_or_c)
    #---------------------------------------

    #3
    not_or_not_or = Expr('|',not_a,not_b,c)
    #----------------------------------------

    return Expr("&",a_or_b,iff,not_or_not_or)
    "*** END YOUR CODE HERE ***"
...
def sentence2() -> Expr:
    ...
    notb = Expr('~',b)
    notd = Expr('~',d)
    notbANDnotd = Expr('&',notb,notd)
    two = Expr('>>',a,notbANDnotd)
    # -------------------------------
    
    #3
    notc = Expr('~',c)
    BandNOTC = Expr('&',b,notc)
    notBandNOTC = Expr('~',BandNOTC)
    three = Expr('>>',notBandNOTC,a)
    # -------------------------------
    ...
    return Expr('&',one,two,three,four)
    "*** END YOUR CODE HERE ***"
...
def sentence3() -> Expr:
    ...
    alive0andnotkilled1 = Expr("&",PacmanAlive_0,Expr('~',PacmanKilled_0))
    notalive0adborn0 = Expr("&",Expr('~',PacmanAlive_0),PacmanBorn_0)

    orr = Expr("|",alive0andnotkilled1,notalive0adborn0)
    first = Expr('<=>',PacmanAlive_1,orr)

    opposite_second = Expr('&',PacmanAlive_0,PacmanBorn_0)
    second = Expr('~',opposite_second)

    all_together = Expr('&',first,second,PacmanBorn_0)
...
def entails(premise: Expr, conclusion: Expr) -> bool:
    ...
    find = findModel(Expr('~',conclusion))
    ret = pl_true(premise,find)
    if ((ret == False)):
        return True
    return False
...
def plTrueInverse(assignments: Dict[Expr, bool], inverse_statement: Expr) -> bool:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    return pl_true(Expr('~',inverse_statement),assignments)
    "*** END YOUR CODE HERE ***"
...
def atLeastOne(literals: List[Expr]) -> Expr:
    ...
    # A or B or ... or Last
    length = len(literals)
    myExpr = Expr('|',literals[0],literals[1])
    
    for i in range(2,int(length)):
        myExpr = Expr('|',myExpr,literals[i])
    
    return to_cnf(myExpr)
    "*** END YOUR CODE HERE ***"
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    # (A & B) | (C & D) | .... | (X & Z)
    comb_list = list(itertools.combinations(literals,2))
    ...
    And1 = Expr('&',n00,n01)
    And2 = Expr('&',n10,n11)
    myExpr = Expr('|',And1,And2)    
    for i in range(2,int(length)):
        n00 = comb_list[i][0]
        n01 = comb_list[i][1]
        And1 = Expr('&',n00,n01)
        myExpr = Expr('|',myExpr,And1)
    
    #if length not int , then take the last element
    if (length/2 != int(length/2)):
        n00 = comb_list[int(length/2)+1][0]
        n01 = comb_list[int(length/2)+1][1]
        And1 = Expr('&',n00,n01)
        myExpr = Expr('|',myExpr,And1)

    return to_cnf(Expr('~',myExpr))
...
def exactlyOne(literals: List[Expr]) -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    atleastone = atLeastOne(literals)
    atmostone = atMostOne(literals)
    return to_cnf(Expr('&',atleastone,atmostone))
    "*** END YOUR CODE HERE ***"
...
def pacmanSuccessorAxiomSingle(x: int, y: int, time: int, walls_grid: List[List[bool]]=None) -> Expr:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    return Expr('<=>',PropSymbolExpr(pacman_str, x, y, time=now),disjoin(possible_causes))
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    for x,y in all_coords:
        if walls_grid[x][y] == True or walls_grid[x][y] == 1 :
    ...
    for x,y in non_outer_wall_coords:
        pacmanPosition.append(PropSymbolExpr(pacman_str, x, y, time =t))
    pacphysics_sentences.append(exactlyOne(pacmanPosition))
    #-----------

    # third task
    pacphysics_sentences.append(exactlyOne([PropSymbolExpr('South',time=t),
                                            PropSymbolExpr('North',time=t),
    ...
    #forth task
    if sensorModel != None:
        smResult = sensorModel(t+1,non_outer_wall_coords)
        pacphysics_sentences.append(smResult)

    #----------

    #fifth task
    if successorAxioms != None:
        saResult = successorAxioms(t+1,walls_grid,non_outer_wall_coords)
        pacphysics_sentences.append(saResult)
    #----------
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    model11 = findModel(Expr('&',conjoin(KB),PropSymbolExpr(pacman_str,x1,y1,time=1)))
    model12 = findModel(Expr('&',conjoin(KB),~PropSymbolExpr(pacman_str,x1,y1,time=1)))
...
def positionLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        KB.append(exactlyOne(pacmanPosition))
        y= findModel(Expr('&',conjoin(KB),PropSymbolExpr(pacman_str,xg,yg,time=t)))
        if y != False:
            list1 = extractActionSequence(y,actions)
            if len(list1) != 0:
                return list1

        KB.append(exactlyOne([PropSymbolExpr('South',time=t),
                              PropSymbolExpr('North',time=t),
                              PropSymbolExpr('East',time=t),
                              PropSymbolExpr('West',time=t)]))
        
        for x1,y1 in non_wall_coords:
            KB.append(pacmanSuccessorAxiomSingle(x1,y1,t+1,walls_grid))
...
def foodLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        for x,y in non_wall_coords:
            if (x,y) in food:
                pacmanFood.append(PropSymbolExpr(food_str,x,y,time=t))
            else:
                pacmanFood.append(~PropSymbolExpr(food_str,x,y,time=t))

        KB.append(conjoin(pacmanFood))
        pacmanNoFood = list()
        for x,y in non_wall_coords:
                pacmanNoFood.append(~PropSymbolExpr(food_str,x,y,time=t))
        # -----------------
        pacmanPosition = list()
        for x,y in non_wall_coords:
            pacmanPosition.append(PropSymbolExpr(pacman_str, x, y, time =t))
        ...
        KB.append(exactlyOne(pacmanPosition))
        y= findModel(Expr('&',conjoin(KB),conjoin(pacmanNoFood)))
        if y != False:
            list1 = extractActionSequence(y,actions)
            if len(list1) != 0:
                return list1
        ...
        for x1,y1 in non_wall_coords:
            KB.append(pacmanSuccessorAxiomSingle(x1,y1,t+1,walls))
            # -- Food Successor Axiom
            pacmanNotAtxy = Expr('~',PropSymbolExpr(pacman_str,x,y,time=t))
            foodAtxy = PropSymbolExpr(food_str,x,y,time=t)
            KB.append(Expr('&',PropSymbolExpr(food_str,x,y,time=t+1),Expr('&',foodAtxy,pacmanNotAtxy)))
            # -----------------------
