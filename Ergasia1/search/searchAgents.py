...
class CornersProblem(search.SearchProblem):
    ...
    class State:
        ...
        def __init__(self,position,visited_corn=[False,False,False,False]):
            self.position = position
            self.visited_corners = visited_corn
            # return self.position

        def __eq__ (self,another):
            return (self.position == another.position) # and (self.visited_corners == another.visited_corners)
    ...
    def __init__(self, startingGameState):
        ...
        self.startingPosition = startingGameState.getPacmanPosition() #(4,5)
        # print(self.startingPosition, "start pos")
        ...
        for corner in self.corners:
            if startingGameState.hasFood(*corner): 
                self.goalcorners.append(corner)
    ...
    def isGoalState(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        if (len(self.goalexpanded) == len(self.goalcorners)- 1) and (state.position in self.goalcorners ) :
            print("its a goal")
            return True
        else:
            print("///////////////////////////////////////////////////////", len(self.goalexpanded), "expanded and ", len(self.goalcorners) ,"goal corn") 
            return False
    ...
    def expand(self, state):
        ...
        gcorners = []
        for gc in self.goalcorners:
            gcorners.append(CornersProblem.State(gc))
            print("gcorner", gcorners[-1].position)

        # if state in gcorners:
        #     print("goallll")
        #     self.goalexpanded.append(state)
        ...
        for action in self.getActions(state):
            ...
            "*** YOUR CODE HERE ***"
            print(self.getNextState(state,action).position,"next States")
            walls = []
            for wall in self.walls:
                walls.append(CornersProblem.State(wall))
            if self.getNextState(state,action) not in walls:
                kid = self.getNextState(state,action)
                children.append((kid,action,self.getActionCost(state,action,kid)))
    ...
    def getNextState(self, state, action):
        ...
        dx, dy = Actions.directionToVector(action)
        nextx, nexty = int(x + dx), int(y + dy)
        "*** YOUR CODE HERE ***" #if its possible
        return CornersProblem.State((nextx,nexty))
...
def cornersHeuristic(state, problem):
    ...
    Distance = 1
    for corn in corners:
        if problem.isGoalState(corn):
            num.update({corn:0})

    for corn in corners:
        if not problem.isGoalState(corn):
            continue
        node = frontier.push(corn)
        while(not frontier.isEmpty()):
            Distance += 1
            for act in problem.getActions(node):
                kid = problem.getNextState(node,act)
                if kid in walls:
                    continue
                frontier.push[kid]
                if num[kid] < Distance:
    ...
    #bres targets, apo to kathena bres ta paidia tou kai bale tin timh toys ==1, meta sta paidia tvn paidivn 2 klp, otan mia timh pame na thn kataxwrisoume kai h nea_timh < palia_timh => timh = nea_timh
    return num.get(state) # Default to trivial solution
